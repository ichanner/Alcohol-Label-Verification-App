...
# application usually just names the country, and the same country goes by
# several names ("Holland", "Burma"). Strip the boilerplate, then resolve
# both sides against a vendored country-name dataset (labelcheck/countries.py,
# regenerated by scripts/make_country_aliases.py). A hard mismatch needs BOTH
# sides recognized as different countries; anything the dataset doesn't know
# (a region like "Scotland", a misread) is a judgment call, never a fail.
...
def _origin_key(text: str) -> str:
    return _ORIGIN_BOILERPLATE.sub("", _clean(text)).strip().casefold()
...
def check_origin(expected: str | None, found: str | None) -> dict:
    ...
    want, got = COUNTRY_ALIASES.get(e), COUNTRY_ALIASES.get(f)
    if e == f or (want and want == got):
    ...
    if want and got:
        return _result(field, label, MISMATCH, expected, found,
                       [f"Different countries: the application says {want}, "
                        f"the label shows {got}."])
    result = check_text(field, label, expected, found)
    if result["status"] == MISMATCH:
        # one side isn't a country name we recognize — that's not a confident
        # conflict, it's a question for a human
        return _result(field, label, REVIEW, expected, found,
                       ["Couldn't confidently read both sides as country names "
                        "— check by eye."])
    return result
